...
def main():
    ...
    wandb.init(
        project=getattr(args, "wandb_project", "FontDiffuser"),
        name=getattr(args, "experience_name", None),
    ...
    print(f"Number of training samples: {len(train_font_dataset)}")
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    ...
                    if global_step % args.ckpt_interval == 0:
                        ...
                        unwrapped = accelerator.unwrap_model(model)  # ここ重要

                        torch.save(unwrapped.unet.state_dict(), f"{save_dir}/unet.pth")
                        torch.save(
                            unwrapped.style_encoder.state_dict(),
                            f"{save_dir}/style_encoder.pth",
                        )
                        torch.save(
                            unwrapped.content_encoder.state_dict(),
                            f"{save_dir}/content_encoder.pth",
                        )

                        # “totalmodel” 相当：まとめて保存したいなら state_dict を束ねる
                        torch.save(
                            {
                                "unet": unwrapped.unet.state_dict(),
                                "style_encoder": unwrapped.style_encoder.state_dict(),
                                "content_encoder": unwrapped.content_encoder.state_dict(),
                                "global_step": global_step,
                            },
                            f"{save_dir}/model_components.pth",
                        )
                ...
                #             f"{save_dir}/content_encoder.pth",
                #         )
                #         torch.save(model, f"{save_dir}/totalmodel.pth")
                #         logging.info(
